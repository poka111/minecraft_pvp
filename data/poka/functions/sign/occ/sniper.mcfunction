...
#看板のgiveコマンド
#/give @s minecraft:oak_sign{BlockEntityTag:{Text1:'[{"text":"","clickEvent":{"action":"run_command","value":"function poka:choose_sniper"}},{"text":"\\u30b9\\u30ca\\u30a4\\u30d1\\u30fc","bold":true,"underlined":true,"color":"dark_aqua"},{"text":"\\u3092\\u9078\\u629e"}]'},display:{Name:'{"text":"Custom Sign"}'}}
#設置コマンド
#/setblock ~ ~1 ~ oak_wall_sign[facing = west]{Text1:'[{"text":"","clickEvent":{"action":"run_command","value":"function poka:choose_sniper"}},{"text":"\\u30b9\\u30ca\\u30a4\\u30d1\\u30fc","bold":true,"underlined":true,"color":"dark_aqua"},{"text":"\\u3092\\u9078\\u629e"}]'}

execute as @s[scores={chose = 1}] run title @s[scores={chose = 1}] title "あなたは職業を選択済みです"
execute as @s[scores={chose = 0}] run scoreboard players set @s[scores={chose = 0}] sniper 1
...
tag @s add ready
execute as @s[scores={sniper = 1}] run tp @s -4.5 -22 28.5
title @a[scores={sniper = 1}] actionbar {"text":"スナイパーを選択しました","color":"yellow"}